...
//#endif
    ...
    find_if(begin(mapEnvList), end(mapEnvList), [&](auto pr) { return (pr.first == L"HTTP_AUTHORIZATION") ? strAuthorization = pr.second, true : false;  });
            ...
            if (strDepth == L"1" || strDepth == L"infinity")
            {
                for (auto& p : fs::directory_iterator(strRootPath + strPath, ec))
                {
                    ...
                    bool bIsDir = ((fs::is_directory(p, ec) == true && ec == error_code())
                                    || (_wstat64(FN_STR(p.path().wstring()).c_str(), &stFileInfo) == 0 && stFileInfo.st_mode & S_IFDIR));
